...
public class ScaleAnimation : PremadeAnimation
{
    ...
    public ScaleAnimation(MonoBehaviour obj, Vector3 to, Nullable<Vector3> from = null, float duration = 1f)
    {
    ...
    {
        obj.transform.localScale += this.origin;
        while (this.Progress < 1f)
        {
            yield return new WaitForSeconds(FRAME_DELAY);
            obj.transform.localScale += GetFrameAmountFor(this.final - this.origin);
